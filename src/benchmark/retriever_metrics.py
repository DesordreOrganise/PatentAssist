...
def benchmark_retriever(retriever: Retriever, dataset: Optional[pd.DataFrame] = None):
    ...
    logging.info(f"{code_Green}Average CPU energy: {
                 np.mean(measurable_retriever.carbone['cpu'])} kWh{code_end}")
    logging.info(f"{code_Green}Average GPU energy: {
                 np.mean(measurable_retriever.carbone['gpu'])} kWh{code_end}")
    logging.info(f"{code_Green}Average emissions: {
                 np.mean(measurable_retriever.carbone['emission'])} kgCO2e{code_end}")
