...
from .nDCG import NDCG_articles, NDCG_rules
import pytest


def test_NDCG_4_articles():
    k = 4
    metric = NDCG_articles(k)
...
def test_NDCG_4_articles_2():
    ...
    assert precision == (((1 / log2(2 + 0)) + (1 / log2(2+2))) / (1 + (1 / log2(2+1))))
...
def test_NDGC_K_rules_2():
    ...
    assert precision == (((1 / log2(2 + 1)) + (1 / log2(2+2))) / (1 + (1 / log2(2+1))) )
