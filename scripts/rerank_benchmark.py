...
class Benchmark_retriever(Retriever):
    ...
    @codecarbone_fr
    @measure
    def measureable_rerank(self, query: str, docs: list[Document]) -> list[Document]:
        docs = self.retriever.reranker.rerank(query, docs, k=4)
        return docs

    def measureable_retrieve_documents(self, query: str):
        docs = self.retriever.retrieve_documents(query, self.rerank)
        (docs, time), (emission, cpu, gpu) = self.measureable_rerank(query, docs)
        return (docs, time), (emission, cpu, gpu)
...
def benchmark_retriever(retriever: Retriever, dataset: Optional[pd.DataFrame] = None, rerank: bool = True):
    ...
    output_json = {
        ...
        "rerank_model": retriever.reranker.model_name,
    ...
    count = sum([1 for file in os.listdir("output/")
                if file.startswith("benchmark_rerank")])
    with open(f"output/benchmark_rerrank_{count}.json", "w") as f:
        json.dump(output_json, f)

    with open(f"output/debug_{count}.json", "w") as f:
        json.dump([str(doc.id) for doc in measurable_retriever.debug], f)
...
if __name__ == "__main__":
    ...
    benchmark_retriever(retriever, rerank=True)
